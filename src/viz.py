...
def piechart_channel_like_count(df_all, channel_ids):
    '''Create a barplot and save the image to a folder. Return image name. Take a dataframe with videodata  as input. Input channel_ids to render image name.'''

    channel_ids_string = '_'.join(channel_ids)
    image_name = f'static/images/{channel_ids_string}_piechart_channel_like_count.png'

    plt.figure(figsize=(FIG_W, FIG_H))
    df_all.groupby('channel_title').size().sort_values(ascending=False).plot.pie()
    plt.xticks(rotation=ROT)
    plt.xlabel("Channel Name")
    plt.ylabel("Like Count")
    plt.title('Like Counts per Channel', fontdict = {'fontsize' : TS})
    plt.savefig(image_name, dpi=100)

    return image_name

'''
def barplot_channel_view_rate(df_all, channel_ids):
    #Create a barplot and save the image to a folder. Return image name. Take a dataframe with videodata  as input. Input channel_ids to render image name.
    ...
    plt.figure(figsize=(FIG_W, FIG_H))
    df_all.groupby('viewer_rating').size().sort_values(ascending=False).plot.bar()
    plt.xticks(rotation=ROT)
    plt.xlabel("Channel Name")
    plt.ylabel("View rate")
    plt.title('View rate per Channel', fontdict = {'fontsize' : TS})
    plt.savefig(image_name, dpi=100)
